...
local ATTACKS_PER_TARGET = config.ATTACKS_PER_TARGET or 6 -- เดิม 10 → ลดจำนวนการยิง
local TELEPORT_DISTANCE_THRESHOLD = config.TELEPORT_DISTANCE_THRESHOLD or 12
local TARGET_RECHECK_DELAY = config.TARGET_RECHECK_DELAY or 2.5 -- เดิม 1.25 → ตรวจช้าลง

local ATTACK_INTERVAL = config.ATTACK_INTERVAL or 0.35 -- เดิม 0.15 → เพิ่มความช้า
...
-- เพิ่มเอฟเฟกต์ hover สำหรับปุ่ม minimize
...
-- ใช้ระบบลากที่ปรับปรุงแล้ว
makeEnhancedDraggable(mainContainer)

-- ======================== ENHANCED BUTTON EFFECTS ========================

-- เพิ่มเอฟเฟกต์สำหรับ toggle button
...
local function startFarm()
    ...
    farmLoop = RunService.Heartbeat:Connect(function()
        ...
        if lastTarget then
            ...
            -- ตรวจสอบ HP ว่ามันลดหรือไม่
            local currentHP = lastTarget:FindFirstChild("Health") and lastTarget.Health.Value or 0

            if lastHP == nil then
                lastHP = currentHP
                stuckStartTime = tick()
            elseif currentHP >= lastHP then
                -- ถ้า HP ไม่ลด
                if tick() - stuckStartTime > 3 then
                    warn("⚠️ Mob HP stuck. Rejoining server...")
                    task.wait(1)
...
-- 💰 Enhanced Auto Sell & Resource Management
task.spawn(function()
    while true do
        ...
        -- อัปเดต Resource Display และเช็คเป้าหมาย
        local reached = true
        for name, goal in pairs(targetResources) do
        ...
        -- ถ้าครบเป้าหมายแล้ว และยังไม่ได้วาร์ป
